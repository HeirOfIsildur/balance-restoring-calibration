...
def load_triangle(filename="triangle_observed.csv"):
    ...
    vy = os.environ.get("RESERVING_VALUATION_YEAR")  # back-test at valuation year I (see data_prep.prepare_data)
    if vy:
        I = int(vy)
        triangle = triangle[:I].copy()
        if "observed" in filename:  # mask the cells beyond calendar year I; the cohort itself stays the deposited one
            for a in range(1, I + 1):
                for k in range(1, cfg.MAX_DEV_LAG + 1):
                    if a + k - 1 > I:
                        triangle[a - 1, k - 1] = np.nan
    return triangle
