...
def prepare_data():
    ...
    # Back-test at an earlier valuation date (RESERVING_VALUATION_YEAR=I, 10 Sep 2026): keep accident years <= I,
    # call a claim IBNR when its calendar reporting year exceeds I, and observe only payments made by calendar year I.
    # Everything downstream (splits, features, the pre-reporting exclusion, the ground truth from the full
    # transactions) then follows from cfg.MAX_ACCIDENT_YEAR = I; the chain-ladder triangles are NOT rebuilt.
    vy = os.environ.get("RESERVING_VALUATION_YEAR")
    if vy:
        I = int(vy)
        cfg.MAX_ACCIDENT_YEAR = I
        claims = claims[claims["accident_year"] <= I].copy()
        claims["ibnr_flag"] = ((claims["accident_year"] + claims["report_dy"] - 1) > I).astype(int)
        transactions = transactions[transactions["accident_year"] <= I].copy()
        transactions["observed_flag"] = (transactions["payment_period"] <= I).astype(int)
        print(f"  [valuation year {I}] claims {len(claims):,} (IBNR {int(claims['ibnr_flag'].sum()):,}), "
              f"observed transactions {int(transactions['observed_flag'].sum()):,}", flush=True)
